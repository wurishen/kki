...
// ===== 调试日志面板（手机可看 API 格式异常原因）=====
try { window.niGetDebugLogs = niGetDebugLogs; window.niClearDebugLogs = niClearDebugLogs; window.niPushDebugLog = niPushDebugLog; } catch (_e) {}
window.niShowDebugLogs = function niShowDebugLogs() {
    try {
        const logs = (typeof niGetDebugLogs === 'function' ? niGetDebugLogs() : (window.__NI_DEBUG_LOGS__ || []));
        const box = document.getElementById('ni-debug-log-panel');
        if (!box) {
            toastr?.info?.('未找到调试面板 DOM');
            return;
        }
        if (!logs.length) {
            box.style.display = 'block';
            box.innerHTML = '<div style="opacity:.7;padding:8px;">暂无日志。触发一次「更新当前偏差」失败后会写到这里。</div>';
            return;
        }
        box.style.display = 'block';
        box.innerHTML = logs.slice().reverse().map(e => {
            const color = e.level === 'error' ? '#f88' : (e.level === 'warn' ? '#fc8' : '#9cf');
            const detail = e.detail ? `<pre style="white-space:pre-wrap;word-break:break-all;margin:4px 0 0;opacity:.85;font-size:11px;">${String(e.detail).replace(/</g,'&lt;')}</pre>` : '';
            return `<div style="border-bottom:1px solid #333;padding:6px 0;"><div style="color:${color};font-size:12px;">[${e.t}] ${String(e.message).replace(/</g,'&lt;')}</div>${detail}</div>`;
        }).join('');
    } catch (err) {
        console.error(err);
        toastr?.error?.('打开调试日志失败: ' + err.message);
    }
};
window.niClearDebugLogsUI = function niClearDebugLogsUI() {
    try { if (typeof niClearDebugLogs === 'function') niClearDebugLogs(); } catch (_) {}
    const box = document.getElementById('ni-debug-log-panel');
    if (box) { box.innerHTML = ''; box.style.display = 'none'; }
    toastr?.success?.('调试日志已清空');
};
